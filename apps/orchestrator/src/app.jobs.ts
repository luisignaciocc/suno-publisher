...
export class AppJobs {
      ...
      const [titleCompletion, tagsCompletion] = await Promise.all([
        this.openai.chat.completions.create({
          messages: [
            ...
            {
              role: 'user',
              content: `Generate a title for a jazz beat`,
            },
          ],
          model: 'gpt-4o-mini',
          temperature: 1.3,
          ...
          messages: [
            {
              role: 'system',
              content: `
              You are an assistant for generating tags for a jazz instrumental beat. Follow these rules for the letter case:
  ...
  async createVideo(
    ...
    // job.log(`Generating DALL-E 3 image...`);
    // const completion = await this.openai.chat.completions.create({
    //   messages: [
    //     {
    //       role: 'system',
    //       content: `
    //       You are an assistant for generating prompts for DALL-E 3 to create anime-style images. Use elements characteristic of anime. The prompt should describe a relaxing, atmospheric, and aesthetically pleasing jazz scene. Return only the prompt without any additional text.
    //       `,
    //     },
    //     {
    //       role: 'user',
    //       content: `Generate a DALL-E prompt for an anime jazz style image`,
    //     },
    //   ],
    //   model: 'gpt-4o-mini',
    // });
